...
async function boot(){
 try{
   const r=await fetch('data/programa.json?v=20260914-ajustes-finales',{cache:'no-store'});
...
function card(t){const s=saved(t.codigo);return `<article class="card" style="--mesa:${colors[t.mesa]||'#c8102e'}"><div class="meta">${t.codigo} · ${t.tipo} · ${t.modalidad}</div><h3>${t.titulo}</h3><p><b>${(t.autores||[]).join('; ')}</b><br>${t.diaTexto} · ${t.horario}<br>${t.mesa} · ${t.sala||''}</p><div class="actions"><button class="small ${s?'saved':''}" onclick="toggle('${t.codigo}')">${s?'★ Guardado':'☆ Mi Agenda'}</button><button class="small" onclick="detail('${t.codigo}')">Ver información completa</button>${t.tipo==='Cartel'?`<button class="small" onclick="openPdf('carteles/${t.codigo}.pdf','El cartel todavía no ha sido cargado.')">Ver cartel</button>`:''}</div></article>`}
function programa(){return header('Consulta por día, mesa o autor','Programa actualizado con la distribución final de salas y horarios.')+cards(filtered())}
...
function constancias(){
 ...
 return `<div class="section-kicker">Constancias</div><h1 class="section-title">Busca tu participación</h1><p class="lead">Localiza tu trabajo por nombre, título o código. Si el PDF aún no está cargado, el sitio te lo indicará.</p><div class="toolbar"><input value="${esc(filters.q)}" placeholder="Nombre, título o código…" oninput="filters.q=this.value;render()"></div>${x.length?`<div class="grid">${x.slice(0,120).map(t=>`<article class="card" style="--mesa:${colors[t.mesa]||'#c8102e'}"><div class="meta">${t.codigo} · ${t.tipo}</div><h3>${t.titulo}</h3><p><b>${(t.autores||[]).join('; ')}</b><br>${t.diaTexto} · ${t.mesa}</p><div class="actions"><button class="small" onclick="detail('${t.codigo}')">Ver participación</button><button class="small" onclick="openConstancia('${t.codigo}','${t.tipo}')">Consultar constancia</button></div></article>`).join('')}</div>`:`<div class="empty">No encontramos una participación con esos datos.</div>`}`;
}
function info(){return `<div class="section-kicker">Información</div><h1 class="section-title">Sede y orientación</h1><div class="info"><div class="panel"><h2>ICEA · UAEH</h2><p>Instituto de Ciencias Económico-Administrativas. San Agustín Tlaxiaca, Hidalgo.</p><p><b>Fechas:</b> 23, 24 y 25 de septiembre de 2026.<br><b>Modalidad:</b> híbrida.</p></div><div class="panel"><h2>Consulta rápida</h2><p>Usa Programa para localizar una participación por autor, título, mesa, día o modalidad. En Mi Agenda puedes guardar actividades sin crear una cuenta.</p><div class="notice">Los carteles y constancias se vinculan por código, evitando modificar el programa cada vez que se agregue un PDF.</div></div></div>`}
async function openPdf(path,pending){
 try{
   const r=await fetch(path,{method:'HEAD',cache:'no-store'});
   if(r.ok){window.open(path,'_blank','noopener');return;}
 }catch(e){}
 message('Archivo pendiente',pending);
}
function openConstancia(code,tipo){
 openPdf(`constancias/${code}.pdf`,'La constancia de esta participación todavía no ha sido cargada.');
}
function message(title,text){
 $('#dialogBody').innerHTML=`<div class="dialog-body"><div class="dialog-head"><div class="kicker">11° Congreso ICEA 2026</div><h2>${title}</h2></div><p>${text}</p></div>`;
 $('#dialog').showModal();
}
function detail(code){
 const t=DATA.trabajos.find(x=>x.codigo===code);if(!t)return;
 $('#dialogBody').innerHTML=`<div class="dialog-body"><div class="dialog-head"><div class="kicker">${t.codigo} · ${t.tipo} · ${t.modalidad}</div><h2>${t.titulo}</h2></div><p><b>Autores:</b> ${(t.autores||[]).join('; ')}</p><p><b>Fecha y hora:</b> ${t.diaTexto} · ${t.horario}<br><b>Mesa:</b> ${t.mesa}${t.mesaNombre?' · '+t.mesaNombre:''}<br><b>Sede:</b> ${t.sala||'Por confirmar'}<br><b>Modalidad:</b> ${t.modalidad}</p><div class="actions"><button class="small ${saved(t.codigo)?'saved':''}" onclick="toggle('${t.codigo}');detail('${t.codigo}')">${saved(t.codigo)?'★ Guardado en Mi Agenda':'☆ Agregar a Mi Agenda'}</button>${t.tipo==='Cartel'?`<button class="small" onclick="openPdf('carteles/${t.codigo}.pdf','El cartel todavía no ha sido cargado.')">Ver cartel</button>`:''}<button class="small" onclick="openConstancia('${t.codigo}','${t.tipo}')">Consultar constancia</button></div></div>`;
 $('#dialog').showModal()
}
