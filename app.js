...
const OPS={
 ...
 '1|Mesa 4':['Dra. María Aline Manzo Martínez','Dra. Lizet Manzo Martínez','Mtro. Edgar Esaúl Vite Vite Gómez'],
 ...
 '2|Mesa 1':['Dr. Martín Aubert Hernández Calzada','Dra. Blanca Cecilia Salazar Hernández','Dr. Eusebio Ortíz Zarco'],
 ...
 '2|Mesa 4':['Dra. María Aline Manzo Martínez','Dra. Nancy Testón Franco','Aura Paulina Flores Barrera'],
 ...
 '2|Bloque virtual de carteles':['—','Dra. Ruth Ortiz Zarco','—'],
 ...
 '3|Mesa 4':['Dra. María Aline Manzo Martínez','Dr. Carlos Antonio Rosas Candelas','Dra. María Aline Manzo Martínez'],
 '3|Bloque virtual':['—','Dra. Judith Alejandra Velázquez Castro','Dra. Aide Maricel Carrizal Alonso']
};

async function boot(){
 try{
  const v='20260914-programa-final';
...
function specialLocation(e){return e.ubicacion||''}
...
function programa(){
 ...
 return `<div class="section-head"><div><div class="eyebrow">Programa académico</div><h1 class="section-title">${day==='all'?'Programa general':'Programa por mesa'}</h1><p class="lead">${day==='all'?'Consulta todas las participaciones de los tres días en una sola vista. También puedes elegir un día específico.':'Cada mesa conserva su horario, sede, moderación y apoyo.'}</p></div></div>
 ${dayTabs()}
 <div class="filters simple"><select onchange="filters.mesa=this.value;render()"><option value="all">Todas las mesas / bloques</option>${DATA.mesas.map(m=>`<option value="${esc(m.codigo)}" ${filters.mesa===m.codigo?'selected':''}>${esc(m.codigo)}</option>`).join('')}</select><select onchange="filters.modalidad=this.value;render()"><option value="all">Todas las modalidades</option><option ${filters.modalidad==='Presencial'?'selected':''}>Presencial</option><option ${filters.modalidad==='Virtual'?'selected':''}>Virtual</option></select></div>
 ${day==='all'?programaAll():programaDay(day)}`;
}
function dayTabs(){return `<div class="day-tabs general-tabs">${[['all','Programa','General'],['1','Miércoles','23 septiembre'],['2','Jueves','24 septiembre'],['3','Viernes','25 septiembre']].map(d=>`<button class="day-tab ${filters.day===d[0]?'active':''}" onclick="filters.day='${d[0]}';render()"><span>${d[1]}</span><b>${d[2]}</b></button>`).join('')}</div>`}
function filtered(day=filters.day){const q=norm(filters.q);return DATA.trabajos.filter(t=>(String(day)==='all'||String(t.dia)===String(day))&&(filters.mesa==='all'||t.mesa===filters.mesa)&&(filters.modalidad==='all'||t.modalidad===filters.modalidad)&&(!q||norm([t.codigo,t.titulo,(t.autores||[]).join(' '),t.mesa,t.sala,t.diaTexto].join(' ')).includes(q)))}
...
function programaDay(day){const items=filtered(day),specials=specialsForDay(day);return `${specials.length?`<section class="day-specials">${specials.map(specialCompact).join('')}</section>`:''}${mesaBlocks(day,items)}`}
function programaAll(){return ['1','2','3'].map(day=>{const items=filtered(day),specials=specialsForDay(day);if(!items.length&&!specials.length)return '';const label=day==='1'?'Miércoles 23 de septiembre':day==='2'?'Jueves 24 de septiembre':'Viernes 25 de septiembre';return `<section class="general-day"><div class="general-day-head"><span>${esc(label)}</span></div>${specials.length?`<div class="day-specials">${specials.map(specialCompact).join('')}</div>`:''}${mesaBlocks(day,items)}</section>`}).join('')||`<div class="empty">No hay resultados con esos filtros.</div>`}
...
function mesaBlock(day,mesa,works,open){
 ...
 const span=blockSpan(day,mesa,works); const title=first.mesaNombre&&first.mesaNombre!==mesa?first.mesaNombre:mesa;
 ...
 ${op?`<div class="roles"><div><span>Líder de mesa</span><b>${esc(op[0])}</b></div><div><span>Moderador/a</span><b>${esc(op[1])}</b></div><div><span>Apoyo</span><b>${esc(op[2])}</b></div>${zoom?`<div class="zoom-role"><span>Sesión virtual</span><a class="join-zoom" href="${esc(zoom)}" target="_blank" rel="noopener">Entrar a la sesión</a></div>`:''}</div>`:zoom?`<div class="roles"><div class="zoom-role"><span>Sesión virtual</span><a class="join-zoom" href="${esc(zoom)}" target="_blank" rel="noopener">Entrar a la sesión</a></div></div>`:''}
 <div class="mesa-works">${works.map(workRow).join('')}</div></details>`;
}
function blockSpan(day,mesa,works){const fixed={'1|Mesa 1':'12:00–14:00','1|Mesa 2':'12:15–14:00','1|Mesa 3':'14:15–16:00','1|Mesa 4':'14:15–15:45','1|Mesa 5':'12:00–13:00','1|Bloque virtual':'14:00–15:45','2|Sesión de carteles':'09:00–12:00','2|Mesa 1':'09:00–10:45','2|Mesa 3':'09:00–10:00','2|Mesa 4':'11:00–12:00','2|Mesa 5':'11:00–12:00','2|Bloque virtual':'11:00–12:30','2|Bloque virtual de carteles':'12:00–13:45','3|Mesa 1':'09:00–10:45','3|Mesa 3':'09:00–10:00','3|Mesa 4':'09:00–10:00','3|Bloque virtual':'09:00–10:45'};return fixed[`${day}|${mesa}`]||works[0]?.horario||''}
function keyOf(t){return t.uid||t.codigo||norm(t.titulo)}
function workRow(t){const k=keyOf(t),s=saved(k);return `<article class="work-row"><time>${esc(t.horario)}</time><div><h4>${esc(t.titulo)}</h4><p>${(t.autores||[]).map(esc).join('; ')}</p></div><div class="session-actions"><button class="chip-btn ${s?'saved':''}" onclick="event.preventDefault();toggle('${esc(k)}')">${s?'★ Guardado':'☆ Mi Agenda'}</button><button class="chip-btn" onclick="event.preventDefault();detailByKey('${esc(k)}')">${t.semblanza?'Detalles + semblanza':'Ver detalles'}</button></div></article>`}
function detailByKey(key){const t=DATA.trabajos.find(x=>keyOf(x)===key);if(t)detailObj(t)}

function agenda(){const ids=getAgenda(),x=DATA.trabajos.filter(t=>ids.includes(keyOf(t)));return `<div class="section-head"><div><div class="eyebrow">Mi Agenda</div><h1 class="section-title">Lo que no quieres perderte</h1><p class="lead">Tus selecciones se guardan únicamente en este navegador.</p></div></div>${x.length?`<div class="agenda-list">${x.sort((a,b)=>a.dia-b.dia||timeKey(a.horario)-timeKey(b.horario)).map(t=>`<div class="agenda-day"><b>${esc(t.diaTexto)} · ${esc(t.horario)} · ${esc(t.mesa)}</b>${workRow(t)}</div>`).join('')}</div>`:`<div class="agenda-empty"><h3>Aún no has guardado actividades</h3><p>En Programa selecciona “☆ Mi Agenda” en las participaciones que quieras seguir.</p><button class="text-link" onclick="go('programa')">Explorar programa →</button></div>`}`}
...
function detail(code){const t=DATA.trabajos.find(x=>x.codigo===code);if(!t)return;detailObj(t)}
function detailObj(t){$('#dialogBody').innerHTML=`<div class="dialog-head"><div class="eyebrow">${esc(t.codigo)} · ${esc(t.tipo)}</div><h2>${esc(t.titulo)}</h2></div><div class="dialog-content"><p><b>Autores:</b> ${(t.autores||[]).map(esc).join('; ')}</p><p><b>Fecha:</b> ${esc(t.diaTexto)} · ${esc(t.horario)}<br><b>Mesa / bloque:</b> ${esc(t.mesa)}<br><b>Sede:</b> ${esc(t.sala||'')}<br><b>Modalidad:</b> ${esc(t.modalidad)}</p>${t.zoom?`<p><a class="join-zoom" href="${esc(t.zoom)}" target="_blank" rel="noopener">Entrar a la sesión de Zoom</a></p>`:''}${t.semblanza?`<div class="bio-box"><div class="eyebrow">Semblanza</div><p>${esc(t.semblanza)}</p></div>`:''}</div>`;$('#dialog').showModal()}
