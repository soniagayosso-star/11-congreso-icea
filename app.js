...
function programa(){
  ...
  ${dayTabs()}
  <div style="margin:14px 0 16px;padding:13px 16px;border:1px solid #e7dde0;border-radius:14px;background:#faf7f8;color:#51464a;font-size:14px;line-height:1.45"><b style="color:#85002b">Sesiones virtuales:</b> los enlaces de acceso estarán disponibles en este sitio <b>a partir del 23 de septiembre de 2026</b>.</div>
...
function constancias(){const q=norm(filters.q),x=DATA.trabajos.filter(t=>!q||norm([t.codigo,t.titulo,(t.autores||[]).join(' ')].join(' ')).includes(q));return `<div class="section-head"><div><div class="eyebrow">Constancias</div><h1 class="section-title">Encuentra tu participación</h1><p class="lead">Busca por nombre, título o código.</p></div></div><div style="margin:0 0 18px;padding:13px 16px;border:1px solid #e7dde0;border-radius:14px;background:#faf7f8;color:#51464a;font-size:14px;line-height:1.45"><b style="color:#85002b">Disponibilidad de constancias:</b> podrán consultarse y descargarse <b>a partir del 28 de septiembre de 2026</b>.</div><div class="const-search"><input value="${esc(filters.q)}" placeholder="Nombre, título o código…" oninput="filters.q=this.value;render()"></div><div class="const-list">${x.slice(0,80).map(t=>`<article class="const-item"><div class="code">${esc(t.codigo)}<br>${esc(t.tipo)}</div><div><h4>${esc(t.titulo)}</h4><p>${(t.autores||[]).map(esc).join('; ')}</p></div><div class="session-actions"><button class="chip-btn" onclick="detail('${esc(t.codigo)}')">Ver participación</button><button class="chip-btn" onclick="openConstancia('${esc(t.codigo)}','${esc(t.tipo)}')">Constancia</button></div></article>`).join('')}</div>`}
...
function openConstancia(code,tipo){
  const available=new Date('2026-09-28T00:00:00-06:00');
  if(Date.now()<available.getTime()){
    $('#dialogBody').innerHTML=`<div class="dialog-head"><div class="eyebrow">Constancias</div><h2>Disponibles a partir del 28 de septiembre</h2></div><div class="dialog-content"><p>Las constancias de participación podrán consultarse y descargarse en este sitio a partir del 28 de septiembre de 2026.</p></div>`;
    $('#dialog').showModal();return;
  }
  const folder=tipo==='Cartel'?'carteles':'ponencias';openPdf(`constancias/${folder}/${code}.pdf`,'La constancia todavía no ha sido incorporada al sitio.');
}
