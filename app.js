...
const colors={'Mesa 1':'#78001f','Mesa 2':'#174f86','Mesa 3':'#0f7f78','Mesa 4':'#c8102e','Mesa 5':'#a76400','Sesión de carteles':'#6b4b8a','Bloque virtual':'#315b7d','Bloque virtual de carteles':'#6b4b8a'};
const norm=s=>String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();

async function boot(){
 try{
   const req=[fetch('data/meta-20260907.json?v=1',{cache:'no-store'}),...Array.from({length:6},(_,i)=>fetch(`data/works-20260907-${i+1}.json?v=1`,{cache:'no-store'})),fetch('data.json?v=20260907-bios',{cache:'no-store'}).catch(()=>null)];
   ...
   // Conserva las semblanzas ya integradas al dashboard anterior, sin alterar los datos del programa final.
   if(legacy&&legacy.ok){
 ...
 }catch(e){console.error(e);$('#app').innerHTML=`<div class="error"><b>No se pudo cargar el programa.</b><br>${e.message}</div>`;}
}
function bind(){
...
function render(){const a=$('#app');if(view==='inicio')a.innerHTML=inicio();else if(view==='programa')a.innerHTML=programa();else if(view==='agenda')a.innerHTML=agenda();else if(view==='eventos')a.innerHTML=eventos();else if(view==='carteles')a.innerHTML=carteles();else if(view==='constancias')a.innerHTML=constancias();else a.innerHTML=info();updateCount();}
function inicio(){const t=DATA.evento.totales||{};return `<section class="hero"><div class="kicker">Plataforma oficial</div><h1>11° Congreso Internacional ICEA 2026</h1><p>Programa académico, eventos destacados, carteles, constancias y orientación para asistentes en una sola plataforma.</p><div class="actions"><button class="btn primary" onclick="go('programa')">Ver programa</button><button class="btn" onclick="go('eventos')">Eventos destacados</button></div></section><section class="stats"><div class="stat"><div id="countdown" class="countdown"></div></div><div class="stat"><b>${t.trabajos||DATA.trabajos.length}</b><span>Trabajos</span></div><div class="stat"><b>${t.mesas||5}</b><span>Mesas</span></div><div class="stat"><b>${t.ponencias||0}</b><span>Ponencias</span></div><div class="stat"><b>${t.carteles||0}</b><span>Carteles</span></div></section><div class="section-kicker">Eventos destacados</div><h2 class="section-title">Momentos centrales del Congreso</h2><p class="lead">El cronograma general permanece visible y el programa académico puede consultarse por día, mesa y modalidad.</p>${eventCards((DATA.programaGeneral||[]).filter(e=>/inauguración|conferencia|foro|reunión|clausura|encuentro|expo/i.test(e.actividad)))}<div style="margin-top:28px" class="section-kicker">Cronograma</div><h2 class="section-title">Programa general</h2>${schedule(DATA.programaGeneral||[])}`;}
function header(title,lead){return `<div class="section-kicker">Programa académico</div><h1 class="section-title">${title}</h1><p class="lead">${lead}</p><div class="tabs">${[['all','Todos'],['1','Miércoles 23'],['2','Jueves 24'],['3','Viernes 25']].map(x=>`<button class="tab ${filters.day===x[0]?'active':''}" onclick="filters.day='${x[0]}';render()">${x[1]}</button>`).join('')}</div><div class="toolbar"><input value="${esc(filters.q)}" placeholder="Título, autor, código…" oninput="filters.q=this.value;render()"><select onchange="filters.mesa=this.value;render()"><option value="all">Todas las mesas / bloques</option>${DATA.mesas.map(m=>`<option value="${esc(m.codigo)}" ${filters.mesa===m.codigo?'selected':''}>${m.codigo}</option>`).join('')}</select><select onchange="filters.modalidad=this.value;render()"><option value="all">Todas las modalidades</option><option ${filters.modalidad==='Presencial'?'selected':''}>Presencial</option><option ${filters.modalidad==='Virtual'?'selected':''}>Virtual</option></select></div>`;}
...
function programa(){return header('Consulta por día, mesa o autor','Información correspondiente al programa final del 7 de septiembre de 2026.')+cards(filtered())}
...
function eventos(){return `<div class="section-kicker">Eventos destacados</div><h1 class="section-title">Programa general</h1><p class="lead">Consulta las actividades generales y los bloques académicos del programa final.</p>${eventCards(DATA.programaGeneral||[])}`}
...
function constancias(){const q=filters.q.toLowerCase().trim(),x=DATA.trabajos.filter(t=>!q||[t.codigo,t.titulo,(t.autores||[]).join(' ')].join(' ').toLowerCase().includes(q));return `<div class="section-kicker">Constancias</div><h1 class="section-title">Busca tu participación</h1><p class="lead">Localiza tu trabajo por nombre, título o código. Las constancias se incorporarán a este mismo espacio.</p><div class="toolbar"><input value="${esc(filters.q)}" placeholder="Nombre, título o código…" oninput="filters.q=this.value;render()"></div>${x.length?`<div class="grid">${x.map(t=>`<article class="card" style="--mesa:${colors[t.mesa]||'#c8102e'}"><div class="meta">${t.codigo} · ${t.tipo}</div><h3>${t.titulo}</h3><p><b>${(t.autores||[]).join('; ')}</b><br>${t.diaTexto} · ${t.mesa}</p><div class="actions"><button class="small" onclick="detail('${t.codigo}')">Ver participación</button><button class="small" onclick="openConstancia('${t.codigo}','${t.tipo}')">Consultar constancia</button></div></article>`).join('')}</div>`:`<div class="empty">No encontramos una participación con esos datos.</div>`}`;}
function info(){return `<div class="section-kicker">Información</div><h1 class="section-title">Sede y orientación</h1><div class="info"><div class="panel"><h2>ICEA · UAEH</h2><img class="info-photo" src="assets/images/acceso-icea.jpeg" alt="Acceso ICEA"><p>Instituto de Ciencias Económico-Administrativas. San Agustín Tlaxiaca, Hidalgo.</p><p><b>Fechas:</b> 23, 24 y 25 de septiembre de 2026.<br><b>Modalidad:</b> híbrida.</p><p>Las participaciones virtuales se realizarán mediante Google Meet. Los enlaces de acceso se incorporarán en la siguiente actualización.</p></div><div class="panel"><h2>Croquis de la sede</h2><img class="map-img" src="assets/images/croquis-icea.png" alt="Croquis ICEA"><p>Consulta el programa para identificar edificio, piso y sala de cada participación.</p></div></div>`}
function detail(code){const t=DATA.trabajos.find(x=>x.codigo===code);if(!t)return;$('#dialogBody').innerHTML=`<div class="dialog-body"><div class="dialog-head"><div class="kicker">${t.codigo} · ${t.tipo}</div><h2>${t.titulo}</h2></div><p><b>Autores:</b> ${(t.autores||[]).join('; ')}</p><p><b>Fecha:</b> ${t.diaTexto} · ${t.horario}<br><b>Mesa / bloque:</b> ${t.mesa}${t.mesaNombre&&t.mesaNombre!==t.mesa?' · '+t.mesaNombre:''}<br><b>Sede:</b> ${t.sala||''}<br><b>Modalidad:</b> ${t.modalidad}</p>${t.semblanza?`<div class="bio-box"><div class="section-kicker">Semblanza</div><p>${t.semblanza}</p></div>`:''}</div>`;$('#dialog').showModal()}
